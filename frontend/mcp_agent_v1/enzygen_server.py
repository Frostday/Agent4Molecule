...
def build_enzygen_input(
    ...
    motif_indices: Annotated[list[float], Field(description="Indices of the motif")],
    ...
    try:
        ...
        data = {}
        motif_indices = [int(i) for i in motif_indices]
        ...
        answer_dict["visualize"] = False
        return answer_dict
    
    except ValidationError as e:
        # Convert error into a structured dict
        return {
            "status": "error",
            "errors": e.errors(),   # nice JSON-serializable list of errors
            "answer": str(e),       # fallback full string
            "visualize": False
        }
...
@mcp.tool()
def run_enzygen(input_json: Annotated[str, Field(description="Location of script directory")]) -> dict:
    ...
    answer_dict["answer"] = "Predicted structure(s) from EnzyGen:\n\n" + "\n----------\n".join(output_preds) + "\n----------\n" + "Log File:\n\n" + logs + "\n----------\n" + "Error File:\n\n" + errors
    answer_dict["visualize"] = True
    return answer_dict
